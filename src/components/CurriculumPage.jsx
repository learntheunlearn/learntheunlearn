...
/* ─────────────────────────────────────────────
   ...
   All are purely decorative, pointer-events:none,
   low opacity so they never distract from cards.
───────────────────────────────────────────── */
const IllustrationBook = ({ className }) => (
  <svg viewBox="0 0 120 100" fill="none" xmlns="http://www.w3.org/2000/svg" className={className} aria-hidden="true">
    ...
    <rect x="20" y="22" width="32" height="3" rx="1.5" fill="#7C3AED" opacity="0.3"/>
    <rect x="20" y="30" width="28" height="3" rx="1.5" fill="#7C3AED" opacity="0.3"/>
    <rect x="20" y="38" width="30" height="3" rx="1.5" fill="#7C3AED" opacity="0.3"/>
    <rect x="20" y="46" width="24" height="3" rx="1.5" fill="#7C3AED" opacity="0.3"/>
    {/* second book offset */}
    <rect x="62" y="18" width="46" height="64" rx="5" fill="#8B5CF6" opacity="0.14"/>
    <rect x="66" y="18" width="4" height="64" rx="2" fill="#6D28D9" opacity="0.2"/>
...
const IllustrationPencil = ({ className }) => (
  <svg viewBox="0 0 40 160" fill="none" xmlns="http://www.w3.org/2000/svg" className={className} aria-hidden="true">
    <rect x="10" y="10" width="20" height="120" rx="4" fill="#7C3AED" opacity="0.18"/>
    <rect x="10" y="10" width="20" height="20" rx="4" fill="#A78BFA" opacity="0.3"/>
    <polygon points="10,130 30,130 20,155" fill="#FCD34D" opacity="0.35"/>
    <polygon points="14,138 26,138 20,155" fill="#F59E0B" opacity="0.3"/>
    <rect x="10" y="25" width="20" height="3" fill="#6D28D9" opacity="0.15"/>
  </svg>
);

const IllustrationAPlus = ({ className }) => (
...
const IllustrationMathSymbols = ({ className }) => (
...
const IllustrationCompass = ({ className }) => (
  <svg viewBox="0 0 100 120" fill="none" xmlns="http://www.w3.org/2000/svg" className={className} aria-hidden="true">
    ...
    {/* Center hinge */}
    <circle cx="50" cy="10" r="7" fill="#A78BFA" opacity="0.3"/>
    {/* Arc */}
    <path d="M22 90 Q50 110 78 90" stroke="#8B5CF6" strokeWidth="3" fill="none" opacity="0.2" strokeLinecap="round"/>
    {/* Tips */}
...
const IllustrationRuler = ({ className }) => (
  <svg viewBox="0 0 200 40" fill="none" xmlns="http://www.w3.org/2000/svg" className={className} aria-hidden="true">
...
const IllustrationStar = ({ className }) => (
...
const IllustrationDots = ({ className }) => (
  <svg viewBox="0 0 120 120" fill="none" xmlns="http://www.w3.org/2000/svg" className={className} aria-hidden="true">
    {[0,1,2,3,4].map(row =>
      [0,1,2,3,4].map(col => (
        <circle key={`${row}-${col}`} cx={12 + col*24} cy={12 + row*24} r="3.5" fill="#7C3AED" opacity={(row+col) % 3 === 0 ? "0.2" : "0.10"}/>
      ))
    )}
  </svg>
);
...
export default function CurriculumPage({ type }) {
  ...
  return (
    <div className="relative min-h-screen">
      ...
      >
        {/* Soft lavender-to-white base gradient */}
        <div style={{
        ...
        {/* ── Educational SVG Illustrations ── */}

        {/* TOP-LEFT: Books stack */}
        <IllustrationBook style={{ position: 'absolute', top: '60px', left: '20px', width: '200px', opacity: 0.7 }} className="w-48 absolute top-16 left-4" />

        {/* TOP-LEFT below books: Ruler */}
        <IllustrationRuler className="absolute" style={{ position: 'absolute', top: '200px', left: '10px', width: '180px', opacity: 0.6, transform: 'rotate(-15deg)' }} />

        {/* TOP-RIGHT: A+ grade */}
        <IllustrationAPlus style={{ position: 'absolute', top: '30px', right: '30px', width: '160px', opacity: 0.8 }} />

        {/* TOP-RIGHT below A+: math symbols */}
        ...
        {/* BOTTOM-RIGHT: Math symbols again */}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {curriculumItems.map((item) => (
              <div
                key={item.id}
                className="flex flex-col justify-between p-6 rounded-3xl border border-purple-100/80 hover:-translate-y-1 transition-all duration-300"
                style={{
                  background: 'rgba(255,255,255,0.82)',
                  backdropFilter: 'blur(8px)',
                  WebkitBackdropFilter: 'blur(8px)',
                  boxShadow: '0 4px 24px rgba(109,40,217,0.07), 0 1px 4px rgba(109,40,217,0.04)',
                }}
                onMouseEnter={e => e.currentTarget.style.boxShadow = '0 12px 36px rgba(109,40,217,0.14), 0 2px 8px rgba(109,40,217,0.08)'}
                onMouseLeave={e => e.currentTarget.style.boxShadow = '0 4px 24px rgba(109,40,217,0.07), 0 1px 4px rgba(109,40,217,0.04)'}
              >
                <div>
